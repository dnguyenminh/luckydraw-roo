...
@Transactional
public class GoldenHourRepositoryTest {

    private static final Logger log = LoggerFactory.getLogger(GoldenHourRepositoryTest.class);
    ...
    private GoldenHour createAndSaveGoldenHour(EventLocation eventLocation) {
        ...
        long count = entityManager.createQuery("SELECT COUNT(g) FROM GoldenHour g", Long.class).getSingleResult();
        ...
        log.info("  Expected start >= {} : {}",
            ...
            storedGoldenHour.getStartTime().compareTo(storedGoldenHour.getStartTime().plusSeconds(5)) >= 0);
     ...
     */
    private String formatTimeRangeComparison(String label, LocalDateTime value1, LocalDateTime value2, String comparison) {
        return String.format("%s: %s %s %s (epoch diff: %d)",
            label,
            value1,
            comparison,
            value2,
            value2.toEpochSecond(java.time.ZoneOffset.UTC) - value1.toEpochSecond(java.time.ZoneOffset.UTC));
    }

    /**
     * Helper method to create consistent time ranges with buffer and normalized precision
     */
    ...
    void findByTimeRangeAndStatus_ShouldReturnMatchingGoldenHours() {
        ...
        log.info("  Start: {} (epoch seconds: {})", morningStart, morningStart.toEpochSecond(java.time.ZoneOffset.UTC));
        log.info("  End: {} (epoch seconds: {})", morningEnd, morningEnd.toEpochSecond(java.time.ZoneOffset.UTC));
