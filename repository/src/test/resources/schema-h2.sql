...
DROP INDEX IF EXISTS idx_role_type;
...
DROP INDEX IF EXISTS idx_audit_object_type;
DROP INDEX IF EXISTS idx_audit_object_id;
DROP INDEX IF EXISTS idx_audit_property;
DROP INDEX IF EXISTS idx_audit_created_at;
DROP INDEX IF EXISTS idx_audit_update_time;
DROP INDEX IF EXISTS idx_audit_created_by;
...
CREATE TABLE participants (
    ...
    code VARCHAR(255) NOT NULL UNIQUE,
    name VARCHAR(255) NOT NULL,
    phone VARCHAR(20), -- Added this column for phone
...
CREATE TABLE spin_histories (
    ...
    win BOOLEAN NOT NULL DEFAULT FALSE, -- This must be 'win', not 'is_win'
...
CREATE TABLE configurations (
    ...
    version BIGINT NOT NULL DEFAULT 0,  -- Add missing version column
...
CREATE INDEX idx_audit_object_id ON audit_logs(object_id);
CREATE INDEX idx_audit_property ON audit_logs(property_path);
CREATE INDEX idx_audit_created_at ON audit_logs(created_at);
CREATE INDEX idx_audit_update_time ON audit_logs(update_time);
CREATE INDEX idx_audit_created_by ON audit_logs(created_by);
CREATE INDEX idx_audit_status ON audit_logs(status);

-- H2 Schema specific for tests
-- This file addresses TEXT columns and other H2 dialect specifics

-- Reset sequence if exists
-- DROP SEQUENCE IF EXISTS hibernate_sequence;
-- CREATE SEQUENCE hibernate_sequence START WITH 1 INCREMENT BY 1;

-- Create modified columns for TEXT types
-- The columnDefinition="TEXT" causes problems in H2, so we handle it here
ALTER TABLE IF EXISTS audit_logs ALTER COLUMN old_value VARCHAR(4000);
ALTER TABLE IF EXISTS audit_logs ALTER COLUMN new_value VARCHAR(4000);
ALTER TABLE IF EXISTS audit_logs ALTER COLUMN context VARCHAR(4000);
...
-- Create unique constraints for relationships
...
ALTER TABLE roles ADD CONSTRAINT IF NOT EXISTS uk_roles_role_type UNIQUE (role_type);
