...
CREATE TABLE regions (
    ...
    description TEXT,  -- Add the missing description column
    status VARCHAR(50),
...
CREATE TABLE rewards (
    ...
    reward_value DECIMAL(19,2) NOT NULL DEFAULT 0,  -- Changed from reserved keyword "value" to "reward_value"
    win_probability DECIMAL(5,4) NOT NULL DEFAULT 0,
    status VARCHAR(50),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE participants (
    ...
    address VARCHAR(500),  -- Add the missing address column
    status VARCHAR(50),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    created_by VARCHAR(255),
...
CREATE TABLE spin_histories (
    ...
    reward_id BIGINT,
    golden_hour_id BIGINT,  -- Add the missing golden_hour_id column
    ...
    FOREIGN KEY (golden_hour_id) REFERENCES golden_hours(id) ON DELETE SET NULL  -- Add foreign key constraint
);
...
CREATE INDEX idx_golden_hour_loc ON golden_hours(event_location_id);
...
CREATE INDEX idx_participant_event_loc ON participant_events(event_location_id);
CREATE INDEX idx_participant_event_part ON participant_events(participant_id);
CREATE INDEX idx_participant_event_status ON participant_events(status);
