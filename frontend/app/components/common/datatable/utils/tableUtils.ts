...
export const useDataTable = ({
  ...
  const defaultActions: ActionDef[] = useMemo(() => [
    {
      label: 'Edit',
      onClick: (row: TableRow) => handleEdit(row),
      color: 'blue',
    ...
    {
      label: 'Delete',
      onClick: (row: TableRow) => handleDeleteClick(row.data),
      color: 'red',
  ...
  const effectiveActions = useMemo(() => {
    return (actions || []).filter(action => !action.isTableAction).concat(defaultActions);
