...
import { stringToObjectType } from '../utils'; // Add import for stringToObjectType function
...
export async function fetchRelatedData(
  ...
  const fetchRequest = request || {
    ...
    filters: [],
    search: {},
    objectType: stringToObjectType(sourceEntityType) // Now properly using imported function
  };
...
export async function fetchRelatedTableData(
  ...
  // Use default request if not provided with proper objectType
  const fetchRequest = request || {
    page: 0,
    size: 10,
    sorts: [],
